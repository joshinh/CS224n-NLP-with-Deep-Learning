    ...
    ### YOUR CODE HERE (5-20 lines)
    	...
    	for i in range(l):
    		...
    		for j in range(i - window_size, i + window_size + 1):
    			temp.extend(sentence[j+window_size])

    		temp_tuple = (temp,labels[i])
    		windowed_data.append(temp_tuple)
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(shape=[self.config.n_window_features * self.config.embed_size, self.config.hidden_size],\
        	                       initializer=tf.contrib.layers.xavier_initializer(seed=1), name = 'W')

        b1 = tf.get_variable(shape=[self.config.hidden_size],\
        	                       initializer=tf.contrib.layers.xavier_initializer(seed=2), name = 'b1')

        U = tf.get_variable(shape=[self.config.hidden_size, self.config.n_classes],\
        	                       initializer=tf.contrib.layers.xavier_initializer(seed=3), name = 'U')

        b2 = tf.get_variable(shape=[self.config.n_classes],\
        	                       initializer=tf.contrib.layers.xavier_initializer(seed=4), name = 'b2')
